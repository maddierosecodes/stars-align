...
// Find optimal starting time (t) by looking for the value of (t) when the range of y values is smallest
const findOptimalStartTime = (
  ...
  // Add velocity * step to the y coordinates for the next value of time (t)
  const updatedPointsArray = pointsArray.map((point) => {
    updatedPoint = { ...point };
    updatedPoint.yPos += point.velocityY * step;
    return updatedPoint;
  });
  // Calculate the new range of y coordinates
  const newRangeY =
  ...
  if (newRangeY > rangeY) return time;
